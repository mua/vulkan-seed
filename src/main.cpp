...
public:
	MyWindow(const char* title = "MyWindow", const uint width = 1280, const uint height = 720) : WSIWindow(title, width, height)
	{
...
private:
	void OnResizeEvent(uint16_t width, uint16_t height) {
		// Resize surface, reinitialize swapchain
	}
	void OnCloseEvent() {
		Close();
	}
...
#pragma region VertexBuffer
	...
	auto flushCommandBuffer = [&](vk::CommandBuffer commandBuffer)
	{
		...
		device.waitForFences(1, &fence, VK_TRUE, 100000000000);
...
#pragma endregion
	...
	while (window.ProcessEvents()) {
		...
		if (fpsTimer > 1000.0)
		{
			std::string windowTitle = std::to_string(frameCounter) + " fps";
